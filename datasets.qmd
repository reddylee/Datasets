...
polity <- polity5 |>
  ...
  mutate(
    diff1 = polity5 - dplyr::lag(polity5, 1),
    diff2 = dplyr::lead(polity5, 1)- dplyr::lag(polity5, 1),
...
survival_cox_ph <- survival_data |>
  ...
  drop_na() |>
  select(ccode, country, year, leader, age, startdate, enddate, group, survival_days, status, GDP_trend, GDP_pc, pop_log, polity5, polity_lead1, polity_lead3, polity_lead5, violence)
...
survival_cox_td <- survival_data |>
  ...
  drop_na() |>
  select(id, ccode, country, leader, age, T1, T2, tstart, tstop, group, survival_days, status, GDP_trend, GDP_pc, pop_log, polity5, polity_lead1, polity_lead3, polity_lead5, violence)
...
successful1 <- plm(
  dependent ~ autocoup_seffect + GDP_pc + GDP_trend + pop_log + violence + factor(non_democracy) + factor(cold_war),
  index = c("ccode", "year"),
  model = "within", # fixed effect
  effect = "individual",
  data = coup_autocoup
...
dependent <- coup_autocoup$diff1
successful1 <- plm(
  dependent ~ coup_seffect + GDP_pc + GDP_trend + pop_log + violence + factor(non_democracy) + factor(cold_war),
  index = c("ccode", "year"),
  model = "within", # fixed effect
  effect = "individual",
  data = coup_autocoup
...
dependent <- coup_autocoup$diff3
interactive1 <- plm(
  dependent ~ coup_seffect * factor(cold_war) + autocoup_seffect * factor(cold_war) + GDP_pc + pop_log + factor(non_democracy),
  index = c("ccode", "year"),
  model = "within", # fixed effect
  effect = "individual",
  data = coup_autocoup
...
```{r}
survival_cox_ph1 <- survival_cox_ph |>
  mutate(cold_war = ifelse(year > 1991, 0, 1)) |>
  mutate(group = fct_relevel(group, "B"))

lm(polity_lead1 - polity5 ~ group + GDP_trend + GDP_pc + pop_log + violence + age + cold_war,
  data = survival_cox_ph1
) |>
  summary()

survival_cox_ph2 <- survival_cox_ph |>
  mutate(cold_war = ifelse(year > 1991, 0, 1)) |>
  filter(survival_days > 1065)

survival_cox_ph3 <- survival_cox_ph |>
  mutate(cold_war = ifelse(year > 1991, 0, 1)) |>
  filter(survival_days > 1825)



lm(polity_lead3 - polity5 ~ group * factor(cold_war) + GDP_trend + GDP_pc + pop_log + violence + age,
  data = survival_cox_ph1
) |>
  summary()


lm(polity_lead3 - polity5 ~ group * factor(cold_war) + GDP_trend + GDP_pc + age,
  data = survival_cox_ph2
) |>
  summary()

lm(polity_lead5 - polity5 ~ group * factor(cold_war) + GDP_trend + GDP_pc + age,
  data = survival_cox_ph3
) |>
  summary()

lm(polity_lead3 ~ group,
  data = survival_cox_ph1
) |>
  summary()

lm(polity_lead5 ~ group + year,
  data = survival_cox_ph1
) |>
  summary()
```
...
```{r}
lm(polity_lead3 - polity5 ~ group,
  data = survival_cox_ph1
) |>
  summary()

lm(polity_lead5 - polity5 ~ group,
  data = survival_cox_ph1
) |>
  summary()

summary(survival_cox_ph)

t.test(polity_lead3 ~ group, data = survival_cox_ph1)
```

```{r}
data <- polity5 |>
  summarise(
    avg_polity = mean(polity5, na.rm = T),
    .by = year
  )

data |>
  ggplot(aes(y = avg_polity, x = year)) +
  geom_point()
```

```{r}
library(plm)
plm(polity_lead3 ~ group + GDP_trend + GDP_pc + pop_log + violence + age,
  data = survival_cox_ph1,
  index = "year",
  model = "within"
) |>
...
```{r}
autocoup_index <- autocoup |>
  filter(autocoup_outcome == "Yes") |>
  select(ccode, country, autocoup_date, exit_date) |>
  mutate(exit_year = as.numeric(format(exit_date, "%Y"))) |>
  rowwise() |>
  mutate(years = list(seq(autocoup_date - 1, autocoup_date))) |>
  unnest(years) |>
  select(ccode, country, years, autocoup_date) |> 
  rename(year = years)

autocoup_index |> 
  left_join(polity |> select(ccode, year, polity5)) |> 
  
  rowwise() |> 
  mutate(diff = polity5 - lag(polity5, 1)) |> 
  view()
```
