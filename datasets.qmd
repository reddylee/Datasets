...
stargazer(attempt_model, successful_model, interactive, type = "text")
```
...
```{r}
diff1 <- plm(
  diff1 ~ coup_attemp_3 + autocoup_attemp_3 + GDP_pc + GDP_trend + pop_log + violence + factor(non_democracy) + factor(cold_war),
  index = c("ccode", "year"),
  model = "within", # fixed effect
  effect = "individual",
  data = coup_autocoup
...
diff2 <- plm(
  diff2 ~ coup_attemp_3 + autocoup_attemp_3 + GDP_pc + GDP_trend + pop_log + violence + factor(non_democracy) + factor(cold_war),
  index = c("ccode", "year"),
  model = "within", # fixed effect
  effect = "individual",
  data = coup_autocoup
...
diff3 <- plm(
  diff3 ~ coup_attemp_3 + autocoup_attemp_3 + GDP_pc + GDP_trend + pop_log + violence + factor(non_democracy) + factor(cold_war),
  index = c("ccode", "year"),
  model = "within", # fixed effect
  effect = "individual",
  data = coup_autocoup
...
stargazer(diff1, diff2, diff3, type = "text")
```
...
autocoup_index <- autocoup |>
  ...
  rowwise() |>
  mutate(years = list(seq(autocoup_date - 3, autocoup_date))) |>
  unnest(years) |>
...
#   view()
#
# summary(polity$polity5)
# summary(coup_autocoup$polity5)
#
# polity |>
#   filter(is.na(polity5))
```
