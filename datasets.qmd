...
attempt1 <-
  glm(
    autocoup_attempt ~ regime + GDP_pc + GDP_trend + pop_log + violence + log(cumulative_days_in_office) + factor(cold_war),
    family = binomial(link = "probit"),
    data = autocoup_model |>
      # filter(
      #   regime != "other"
      # ) |>
      mutate(regime = relevel(factor(regime), ref = "dominant-party"))
  )
attempt1 |>
  summary()
...
autocoup_model |>
  select(regime_type) |>
  tabyl(regime_type)
distinct(regime_coup, .keep_all = TRUE)
...
attempt4 <- logistf(
  autocoup_attempt ~ regime_autocoup + GDP_pc + GDP_trend + pop_log + violence + log(cumulative_days_in_office) + age + factor(cold_war),
  data = autocoup_model |>
    filter(
