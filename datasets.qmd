...
pacman::p_load(tidyverse, janitor, data.table, readxl, vdemdata, zoo, plm, democracyData, fuzzyjoin, survival, stargazer, brglm2, survminer, ggsurvfit)
```
...
leader_autocoup <- leader_year |>
  left_join(
    autocoup |>
      ...
      mutate(year = year(extending_date)),
...
leaders_coup <- autocoup_model |>
  ...
  filter(!is.na(coup_date) | autocoup_outcome == 2) |>
  ...
  select(ccode, country, year, leader, startdate, enddate, tstart, tstop, status, group, survival_days, age, regime, violence, GDP_pc, GDP_trend, pop_log, polity5, cold_war)
```

```{r}
#| label: survival_model_data

survival_data <- leaders_coup |>
  filter(
...
survival_cox_ph |>
  ...
  coxph(Surv(survival_days, status) ~ group + regime + GDP_trend + GDP_pc + pop_log + polity5 + violence, data = _) |>
  summary()

survival_cox_td |>
  mutate(group = factor(group, labels = c("Autocoup leaders", "Coup-installed leaders"))) |>
  coxph(Surv(T1, T2, status) ~ group + regime + GDP_trend + GDP_pc + pop_log + polity5 + violence + cluster(id), data = _) |>
  summary()
