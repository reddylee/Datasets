...
write_csv(coup_model, "coup_analysis_model.csv")
```
...
leader_autocoup <- leaders |>
  left_join(autocoup, by = c("ccode", "startdate" = "entry_date")) |>
  mutate(
...
autocoup_model <- leader_autocoup |>
  ...
  left_join(regimes, by = c("ccode", "base_year" = "year")) |>
  group_by(ccode) |>
  fill(regime_autocoup, .direction = "updown") |>
  ungroup() |>
  left_join(political_violence, by = c("ccode", "base_year" = "year")) |>
  mutate(violence = na.locf(violence, .by = ccode)) |>
  left_join(GDP, by = c("ccode", "base_year" = "year")) |>
  filter(!is.na(GDP_pc)) |>
  left_join(polity5, by = c("ccode", "base_year" = "year")) |>
...
write_csv(autocoup_model, "autocoup_analysis_model.csv")
```
...
survival_data <- leaders |>
  left_join(overstay_leaders, by = c("ccode", "startdate" = "entry_date")) |>
  mutate(
