...
demo_model1 <- coup_autocoup |>
  rename(Attempt = diff2, Autocoup = autocoup_effect_lag2, Coup = coup_effect_lag2) |>
  plm(
    Attempt ~ Autocoup + Coup + GDP_pc + GDP_trend + pop_log + violence + factor(non_democracy) + factor(cold_war),
    index = c("ccode", "year"),
    model = "within", # fixed effect
...
demo_model3 <- coup_autocoup |>
  rename(Succeed = diff2, Autocoup = autocoup_seffect_lag2, Coup = coup_seffect_lag2) |>
  plm(
    Succeed ~ Autocoup + Coup + GDP_pc + GDP_trend + pop_log + violence + factor(non_democracy) + factor(cold_war),
    index = c("ccode", "year"),
    model = "within", # fixed effect
...
demo_model1 <- plm(
  Attempt ~ Autocoup + Coup + GDP_pc + GDP_trend + pop_log + violence + regime + factor(cold_war),
  index = c("ccode", "year"),
  model = "within", # fixed effect
  effect = "individual",
  data = coup_autocoup |>
...
demo_model2 <- plm(
  ...
  effect = "individual",
  data = coup_autocoup |>
    rename(Attempt = diff2, Autocoup = autocoup_attemp_lag2, Coup = coup_attemp_lag2) |>
    mutate(
    regime = factor(regime,
      levels = c("dominant-party", "military", "personal", "presidential", "parliamentary", "other")
    )
